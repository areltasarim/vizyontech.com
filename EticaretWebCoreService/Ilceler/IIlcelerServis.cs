...
namespace EticaretWebCoreService
{
    ...
    public partial class IlcelerServis : IIlcelerServis
    {
        private readonly AppDbContext _context;

        private readonly string entity = "İlçe";
        public IlcelerServis(AppDbContext _context)
        {
            this._context = _context;

        }

        public async Task<List<Ilceler>> PageList()
        {
            return (await _context.Ilceler.ToListAsync());
        }

        public async Task<ResultViewModel> UpdatePage(IlceViewModel Model, string submit)
        {
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    else
                    {
                        ...
                        var sayfaGuncelle = _context.Ilceler.Where(p => p.Id == Model.Id).FirstOrDefault();
                        sayfaGuncelle.IlId = Model.IlId;
                        sayfaGuncelle.IlceAdi = Model.IlceAdi;
                        #endregion
        ...
        public async Task<ResultViewModel> DeletePage(IlceViewModel Model)
        {
            var result = new ResultViewModel();
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    var model = _context.Ilceler.Find(Model.Id);
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    if (Deletes != null)
                    {
                        ...
                        {
                            _context.Entry(_context.Ilceler.Find(item)).State = EntityState.Deleted;
                        }
                        await _context.SaveChangesAsync();
